...
- **Completed in code and development; production pending** — MFA إلزامي لمسؤولي الإدارة: تسجيل الدخول يوجّه إلى إعداد TOTP أول مرة أو إلى تحدي الرمز عند وجود عامل موثق، ولا تعرض الواجهة المحمية إلا لجلسة `aal2`. migration أمامية تجعل `private.is_admin()` ترفض `aal1` حتى عند الاتصال المباشر بـData API؛ طُبقت وتحققت على مشروع التطوير (`aal1=false`, `aal2=true`). يلزم نشر التطبيق وmigration معًا، ثم تسجيل عامل المسؤول واختبار الدخول الفعلي قبل إغلاق البند.
- **Completed in production** — Cloudflare Turnstile مفعل على النماذج العامة واختبر بعد إعادة النشر.
- **Pending** — rate limiting للإجراءات العامة الحساسة.
