        ...
        <tbody>
          {responses.map((response) => (
            <tr key={response.id} className="border-t border-[var(--color-border)] align-top">
              <td data-label="الفعالية" className="px-5 py-4 font-bold">{response.eventTitle}</td>
              <td data-label="المشاركة" className="px-5 py-4">{response.attendeeName ?? "مجهول"}</td>
              <td data-label="الضيافة" className="px-5 py-4">{formatArabicNumber(response.hospitalityRating)} / ٥</td>
              <td data-label="المادة" className="px-5 py-4">{formatArabicNumber(response.materialRating)} / ٥</td>
              <td data-label="المقترحات" className="max-w-sm px-5 py-4 whitespace-pre-wrap">{response.suggestions ?? "—"}</td>
              <td data-label="وقت الإرسال" className="px-5 py-4 whitespace-nowrap">{formatArabicDateTime(response.submittedAt)}</td>
            </tr>
          ))}
        </tbody>
      </table>
