    ...
    <fieldset aria-describedby={error ? `${name}-error` : undefined}>
      ...
      <div className="mt-3 flex flex-wrap gap-3" dir="ltr">
        {ratings.map((rating) => (
          <label
            key={rating}
            className="flex size-12 cursor-pointer items-center justify-center rounded-sm border border-[var(--color-border)] bg-white font-bold has-checked:border-[var(--brand-forest)] has-checked:bg-[var(--brand-forest)] has-checked:text-[var(--color-on-primary)]"
          >
      ...
      {error ? (
        <p id={`${name}-error`} role="alert" className="mt-3 text-sm font-bold text-[var(--color-error-text)]">
          {error}
        </p>
      ) : null}
    </fieldset>
    ...
    <form action={formAction} className="form-surface mt-10 grid gap-8 p-5 sm:p-8" noValidate>
      ...
      <fieldset>
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          <label className="flex cursor-pointer items-center gap-3 border border-[var(--color-border)] bg-white p-4 font-bold">
            <input type="radio" name="identityVisible" value="true" required /> نعم، أظهري اسمي
          </label>
          <label className="flex cursor-pointer items-center gap-3 border border-[var(--color-border)] bg-white p-4 font-bold">
            <input type="radio" name="identityVisible" value="false" required /> لا، أرسليه مجهولًا
          </label>
        </div>
        {state.error === "identityVisible" ? (
          <p role="alert" className="mt-3 font-bold text-[var(--color-error-text)]">
            {errors.identityVisible}
          </p>
        ) : null}
      </fieldset>
      {state.error === "save" ? (
      ...
      <button type="submit" disabled={pending} className="button-primary px-6 py-3">
        {pending ? "جارٍ إرسال التقييم…" : "إرسال التقييم"}
      </button>
    </form>
  );
}
