        ...
        <table className="operational-table responsive-admin-table w-full min-w-[56rem] border-collapse text-right text-sm">
          ...
          <thead>
            <tr>
              ...
              <th className="px-5 py-4">الموافقة</th>
              <th className="px-5 py-4">تاريخ التسجيل</th>
            </tr>
          </thead>
          <tbody>
            {contacts.map((contact) => (
              <tr key={contact.id} className="border-t border-[var(--color-border)]">
                ...
                <td data-label="البريد" className="max-w-64 break-all px-5 py-4 md:whitespace-nowrap" dir="ltr">
                  <a className="underline decoration-[var(--brand-olive)] underline-offset-4" href={`mailto:${contact.email}`}>
                    {contact.email}
                  </a>
                </td>
                <td data-label="الموافقة" className="px-5 py-4">
                  <StatusBadge status={contact.unsubscribedAt ? "unsubscribed" : "subscribed"} />
                </td>
                <td data-label="تاريخ التسجيل" className="data-value px-5 py-4 text-xs muted-copy">
                  {formatArabicDateTime(contact.consentedAt)}
                </td>
              </tr>
            ))}
          </tbody>
