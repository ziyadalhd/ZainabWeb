...
export function LoginForm({ errorMessage }: { errorMessage?: string }) {
  ...
  return (
    <form action={loginAction} className="form-surface mt-8 grid gap-5 p-5 sm:p-7">
      ...
      <button className="button-primary min-h-12 px-6 py-3" type="submit">
        دخول المسؤول
      </button>
      <button className="button-secondary min-h-12 px-6 py-3" type="button" onClick={requestRecovery} disabled={isSendingRecovery}>
        {isSendingRecovery ? "جارٍ إرسال الرابط…" : "إرسال رابط تغيير كلمة المرور"}
      </button>
