...
export interface RegistrationTableActions {
  ...
  cancelWaitlistedRegistration: (id: string) => Promise<void>;
  confirmAttendance: (id: string) => Promise<void>;
  recordCheckIn: (id: string, outcome: string) => Promise<void>;
...
function RegistrationActions({ registration, mode, actions }: { registration: Registration; mode: RegistrationTableProps["mode"]; actions: RegistrationTableActions }) {
  const hasSecondaryActions = mode === "current" || (mode === "waitlist" && registration.status !== "cancelled");
  return <div className="flex flex-wrap gap-2"><Link href={`/admin/events/${registration.eventId}?tab=communications`} className="button-primary min-h-10 px-3 py-2 text-sm">فتح التواصل</Link>{mode === "waitlist" && registration.status === "invited" ? <ConfirmActionForm action={actions.revokeInvitation.bind(null, registration.id)} label="سحب الدعوة" confirmation={`هل تريدين سحب دعوة ${registration.attendeeName} وإعادتها لقائمة الانتظار؟`} /> : null}{hasSecondaryActions ? <details className="relative"><summary className="button-secondary min-h-10 cursor-pointer list-none px-3 py-2 text-sm">إجراءات إضافية</summary><div className="mt-2 grid min-w-48 gap-2 rounded-[var(--radius-control)] border border-[var(--color-border)] bg-[var(--color-surface)] p-2 shadow-lg">{mode === "current" && registration.attendanceStatus === "pending" ? <form action={actions.confirmAttendance.bind(null, registration.id)}><button type="submit" className="button-quiet w-full text-right text-sm">تأكيد الحضور</button></form> : null}{mode === "current" && registration.checkInStatus !== "checked_in" ? <form action={actions.recordCheckIn.bind(null, registration.id, "checked_in")}><button type="submit" className="button-quiet w-full text-right text-sm">تسجيل الحضور</button></form> : null}{mode === "current" && registration.checkInStatus !== "absent" ? <ConfirmActionForm action={actions.recordCheckIn.bind(null, registration.id, "absent")} label="تسجيل الغياب" confirmation={`هل أنت متأكدة أن ${registration.attendeeName} لم تحضر الفعالية؟`} /> : null}{registration.status !== "cancelled" ? <ConfirmActionForm action={(mode === "waitlist" ? actions.cancelWaitlistedRegistration : actions.cancelRegistration).bind(null, registration.id)} label="إلغاء التسجيل" confirmation={`هل تريدين إلغاء تسجيل ${registration.attendeeName}؟ لا يمكن التراجع عن هذا الإجراء.`} /> : null}</div></details> : null}</div>;
}

function RegistrationDetails({ registration, mode, actions }: { registration: Registration; mode: RegistrationTableProps["mode"]; actions: RegistrationTableActions }) {
