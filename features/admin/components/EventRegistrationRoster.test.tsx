...
describe("EventRegistrationRoster", () => {
  it("shows the empty state when there are no registrations", () => {
    render(
      <EventRegistrationRoster
        ...
        emptyDescription="ستظهر الأسماء هنا."
      />,
    );
  ...
  it("lists every registration with a working link into the shared registrations workspace", () => {
    ...
    expect(screen.getByRole("link", { name: "فتح" })).toHaveAttribute(
      "href",
      `/admin/registrations?event=${registration.eventId}&view=upcoming&id=${registration.id}`,
    );
    expect(screen.getByRole("link", { name: "إدارة كل التسجيلات لهذه الفعالية" })).toHaveAttribute(
      "href",
      `/admin/registrations?event=${registration.eventId}&view=upcoming`,
    );
  });
