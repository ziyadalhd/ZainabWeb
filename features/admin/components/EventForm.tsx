...
export function EventForm({ action, event, submitLabel, onSaved }: EventFormProps) {
  ...
  useEffect(() => {
    ...
    if (!fieldName) return;
    formRef.current?.querySelector<HTMLElement>(`[name="${fieldName}"]`)?.focus();
  }, [state.error]);

  const fieldError = (field: EventFormActionError) =>
    state.error === field ? <span className="text-sm font-bold text-[var(--color-error-text)]">{errorMessages[field]}</span> : null;

  return (
    <form ref={formRef} action={formAction} onChange={() => setDirty(true)} className="form-surface mt-8 grid max-w-5xl gap-9 p-5 sm:p-8" noValidate>
