...
| Public UX and shared scheduling refinement | **In Progress** | على `codex/public-ux-refinement`: تبويبات قصة النادي، أقرب فعاليتين، حالات تسجيل واضحة، إطار بوستر موحد، تقويم ميلادي عربي RTL، اختيار وقت 12 ساعة بأرباع الساعة، Turnstile مرن، WhatsApp، ورابط المقر القابل للإدارة. في `2026-08-16` أُعيدت قيم المحتوى العام المعتمدة من الإنتاج إلى صف إعدادات مشروع التطوير فقط من دون تغيير الإنتاج. حُسّن منتقي الموعد بأرقام عربية، أسماء أيام مختصرة، نهاية مقترحة بعد ساعتين، وصياغة مثل «من ٦ إلى ٨ مساءً». كما أصبحت جداول الإدارة بطاقات واضحة حتى عرض `1023px`، وأضيف تأكيد قبل الإجراءات الحساسة، وأصبحت عروض الطلبات تنتهي تلقائيًا بعد ٤٨ ساعة مع خيار موعد مختلف. نجح `pnpm lint` و`pnpm typecheck` وVitest (`93/93`) وبناء Next.js `16.2.12`، وفُحصت الإدارة عند `320×568` و`390×844` و`768×1024` و`1440×900` دون تمدد أفقي أو أخطاء متصفح، وتحقق إغلاق التقويم بـEscape وعودة التركيز. لا نشر إنتاجي قبل المعاينة والاعتماد. |
