...
| Administrator TOTP MFA | **Completed in code and development; production pending** | أضيف مسارا `/admin/mfa/setup` و`/admin/mfa/verify` بالعربية وRTL، ويوجه تسجيل الدخول حسب `getAuthenticatorAssuranceLevel()`. يفرض `requireAdmin()` مستوى `aal2` على كل صفحة وإجراء محمي، وتفرض migration `20260815082717_require_admin_mfa_aal2.sql` المستوى نفسه داخل `private.is_admin()` لحماية RLS وRPC مباشرة. نجح ESLint وTypeScript وVitest (`75/75`) وبناء Next.js، وتحقق مشروع Supabase التطويري داخل معاملة متراجعة من رفض `aal1` وقبول `aal2`. لم تطبق migration على الإنتاج بعد حتى لا تُحجب الإدارة قبل نشر الواجهة وتسجيل العامل. |
