...
| Admin password-recovery route | **Completed** | أضيف `/admin/reset-password` كمسار عام محدود يستقبل جلسة الاستعادة المؤقتة فقط، ويعيد تعيين كلمة المرور من المتصفح ثم يلغي الجلسات ويحوّل إلى دخول المسؤول. لا يمرر كلمة المرور إلى Server Action أو يسجلها. يلزم توجيه `Site URL` في Supabase إلى هذا المسار للرسائل المرسلة من Dashboard. |
