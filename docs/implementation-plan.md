...
| Admin schedule and copy refinement | **In progress** | أصبح منتقي موعد الفعالية المنشور يستخدم تاريخًا واحدًا، ساعة، وأربع اختيارات دقائق فقط. وحُسنت في الفرع الحالي صيغة مواعيد طلبات الحجز في الإدارة ورابط المتابعة لتعرض اليوم والتاريخ والوقت بالعربية، مع أعداد عربية وصياغة أوضح لمؤشر السعة؛ نجح lint وtypecheck وVitest (`61/61`) وproduction build، وتبقى معاينة Vercel لهذه الدفعة. |
...
| Production Supabase preflight | **Ready for controlled cutover** | تأكد أن Production هو مشروع `bayn-cultural-club` في `ap-south-1` على PostgreSQL `17.6`، ومشروع التطوير منفصل عنه. تطابقت أول 5 migrations، وحدد `supabase db push --linked --dry-run` 12 migration أمامية متبقية دون seed. الإنتاج يحتوي 3 فعاليات قديمة، و0 تسجيلات، و0 ملفات Storage، وصفًا واحدًا في قائمة المسؤولين. الثلاث جميعها بلا وقت نهاية أو سعر، وإحداها منشورة لكنها انتهت في `2026-08-12` فلا تظهر ضمن الفعاليات القادمة. مراجعة الأمان قبل النقل أظهرت تحذيرات RPC القديمة التي تعالجها migration التقوية المتبقية، إضافة إلى حماية كلمات المرور المسربة المؤجلة. نجحت نسخة `pg_dump` مخصصة لمخطط وبيانات التطبيق (`public`, `private`) بعميل PostgreSQL `18.4`، بصلاحيات ملف `0600` داخل `/private/tmp`، وتحقق فهرسها وبصمتها `SHA-256`; لم تُنسخ Auth أو Storage لأن migrations المتبقية لا تعدل بيانات Auth، ولا توجد ملفات Storage في الإنتاج. لم يُطبق أي migration بعد. |
