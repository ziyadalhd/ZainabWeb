...
| Browser review | **Completed للنطاق العام والإداري المنشور** | نجح مسار التسجيل لليافعات، وتحول السعة المشتقة إلى قائمة انتظار، وتأكيد الحضور، وإلغاء الحجز ثنائي الخطوة مع صفحة نجاح، وقبول دعوة الانتظار مع صفحة نجاح. روجعت واجهة RTL العامة على `1280px` و`390px`. وفي `2026-08-13` روجعت جلسة مسؤول فعلية على Preview للجوال وسطح المكتب عبر النظرة العامة والتقويم والفعاليات وإنشاء فعالية والمسجلين والطلبات والرسائل والانتظار؛ لم يظهر تمدد أفقي أو تحذيرات أو أخطاء في سجل المتصفح، وتحقق بصريًا من منتقي الموعد ذي أرباع الساعة. لم تُرسل نماذج أو تُغيّر بيانات أثناء الفحص. |
| Admin schedule and copy refinement | **Completed** | أصبح منتقي موعد الفعالية يستخدم تاريخًا واحدًا، ساعة، وأربع اختيارات دقائق فقط. تعرض طلبات الحجز اليوم والتاريخ والوقت بالعربية، مع أعداد عربية وصياغة أوضح لمؤشر السعة. نجح lint وtypecheck وVitest (`69/69`) وproduction build، ثم فُحصت Preview الجديدة ونُشرت الدفعة إلى Production. |
| Public UX and shared scheduling refinement | **Completed in production** | تشمل الدفعة تبويبات قصة النادي، أقرب فعاليتين، حالات تسجيل واضحة، إطار بوستر موحد، تقويمًا ميلاديًا عربيًا RTL، اختيار وقت بأرباع الساعة، Turnstile مرنًا، WhatsApp، ورابط المقر القابل للإدارة، إضافة إلى تحسينات الإدارة المتجاوبة. في `2026-08-22` نجح ESLint وTypeScript وكامل Vitest (`126/126`) وبناء Next.js `16.2.12`، ثم نُشرت الدفعة إلى `dpl_4ZQo4BtqHh1FSMzXspVi9S7RPfjy`. نجحت صفحات الرئيسية والفعاليات وحجز المساحة وطلب الورشة والخصوصية والشروط، وتأكد عدم وجود تمدد أفقي عند `390×844` أو أخطاء متصفح. |
| Search metadata and indexing | **Completed in code; indexing gated** | أضيفت Metadata عربية، و`robots.txt`، و`sitemap.xml` للصفحات العامة والفعاليات المنشورة القادمة فقط. تستبعد الخريطة لوحة الإدارة والروابط الآمنة، وتظل الفهرسة محظورة ما لم تكن البيئة Production ويُضبط `SITE_INDEXING_ENABLED=true` صراحة. نجح ESLint وTypeScript وVitest (`69/69`) وبناء Next.js `16.2.12`؛ لم تتغير قاعدة البيانات أو المصادقة أو سلوك الأعمال. |
| Production Supabase cutover and Vercel deployment | **Completed** | في `2026-08-13` تحققنا من النسخة الاحتياطية المقيدة الصلاحيات والبصمة، ثم طبق `supabase db push --linked --yes` migrations الإنتاج الأمامية الاثنتي عشرة دون seed. أصبح سجل الإنتاج مطابقًا للمحلي (`17/17`) وبقيت 3 فعاليات و0 تسجيلات وصف مسؤول واحد ومستخدم Auth واحد و0 ملفات دون فقدان. الجداول العامة الثمانية عليها RLS، والـRPC العامة wrappers غير مميزة الصلاحية، وظهرت مهام Cron الثلاث وbucket `event-posters`. اختفى تحذير RPC الأمني القديم وبقي تحذير حماية كلمات المرور المسرّبة المعطل. نُشرت نسخة Preview بحالة `READY`، ثم نُشر الالتزام نفسه بمتغيرات Production إلى Vercel deployment `dpl_FzsECMnksdxZUFcy54YTxGtkA4JE` وأصبح `bayn-cultural-club.vercel.app` يشير إليه. نجحت طلبات الدخان للرئيسية والفعاليات ودخول الإدارة والحجز والخصوصية والشروط وrobots وsitemap؛ أعاد `/admin` تحويلًا صحيحًا إلى الدخول ولم تظهر أخطاء runtime في فحص الساعة التالية للنشر. ظلت Vercel Authentication والفهرسة العامة مغلقتين. |
| Administrator TOTP MFA | **Completed in production** | أضيف مسارا `/admin/mfa/setup` و`/admin/mfa/verify` بالعربية وRTL، ويوجه تسجيل الدخول حسب `getAuthenticatorAssuranceLevel()`. يفرض `requireAdmin()` مستوى `aal2` على كل صفحة وإجراء محمي، وتفرض migration `20260815082717_require_admin_mfa_aal2.sql` المستوى نفسه داخل `private.is_admin()` لحماية RLS وRPC مباشرة. نجح ESLint وTypeScript وVitest (`75/75`) وبناء Next.js. في `2026-08-15` نُشر الالتزام `b32a075` إلى Production ضمن deployment `dpl_EFArg4JS4RvJjKEpfFAQBVi5QFeU`، وسُجّل عامل TOTP موثق لحساب المسؤول، ثم طُبقت migration على قاعدة الإنتاج وتأكد تسجيلها مباشرة. أثبت اختبار الصلاحية `aal1_is_admin=false` و`aal2_is_admin=true`، ونجحت صفحات النظرة العامة والتقويم والمسجلين الحاليين وإنشاء فعالية بجلسة المسؤول الفعلية من دون أخطاء في سجل المتصفح. |
| Administrator MFA device management — 2026-08-22 | **Completed in production** | أضيف `/admin/security` لإدارة عوامل TOTP من جلسة `aal2`: تسمية الجهاز الجديد، مسح QR والتحقق منه، عرض العوامل الموثقة، ومنع إزالة العامل الأخير. بعد إزالة عامل تُحدّث الجلسة ويعاد تحدي MFA عند انخفاضها إلى `aal1`. تعرض شاشة الدخول اختيار الجهاز عند وجود أكثر من عامل. نجح ESLint وTypeScript وكامل Vitest (`126/126`) وبناء Next.js `16.2.12`، ونُشرت ضمن `dpl_4ZQo4BtqHh1FSMzXspVi9S7RPfjy`. أكد اختبار الإنتاج غير المتلف أن المسار موجود ومحمي؛ لم تُغيّر عوامل MFA الحقيقية أثناء الفحص. لا migration ولا تبعية جديدة. |
| Production quality release — 2026-08-22 | **Completed in production** | طابق فحص Supabase جميع migrations وظيفيًا؛ سجل الإنتاج يستخدم رقمي الوقت المعتمدين `20260820162506` و`20260820210954` بدل رقمي ملفي المصدر المقابلين، لذلك لم يُعد تطبيق DDL. أُنشئ نشر Production مع `--skip-domain`، واجتاز البناء واختبارات الدخان، ثم رُقّي إلى `bayn-cultural-club.vercel.app`. أعادت الصفحات العامة `200`، وأعادت الإدارة والمسار الأمني `307` إلى الدخول، وأعاد المسار المتقاعد `/celebration-booking` الحالة `404` المقصودة. أكد المتصفح جاهزية Turnstile على الدومين الأساسي، ولم تسجل Vercel أخطاء Runtime أو warning/error logs للنشر الجديد. |
| Lean-launch rate limiting and monitoring | **Deferred by owner** | بسبب انخفاض الاستخدام الحالي، أُجل فرض Rate Limiting المخصص وربط Sentry. لم تُثبت حزمة Sentry ولم تُضف متغيرات لها. أنشئت في Vercel Firewall مسودة واحدة غير منشورة لمراقبة طلبات `POST` العامة والحساسة عند تجاوز `30` طلبًا خلال `60` ثانية لكل IP؛ الإجراء عند التجاوز `log` فقط، لذلك لا تمنع أي مستخدم ولا تؤثر على الإنتاج. تبقى Turnstile وحدود Supabase Auth وحماية Vercel DDoS فعالة، ويُستخدم Vercel Runtime Logs مؤقتًا للمراقبة. |
| Final launch smoke — 2026-08-15 | **Completed for non-mutating scope** | نجح `pnpm lint` و`pnpm typecheck` وVitest (`75/75`) وبناء Next.js `16.2.12`. فُحصت على الإنتاج الرئيسية والفعاليات وتفاصيل فعالية وحجز المساحة والحفلات والاستبيانات والخصوصية والشروط، ثم النظرة العامة والتقويم والفعاليات والمسجلون والطلبات والرسائل في الإدارة بجلسة MFA فعلية؛ لم تظهر أخطاء في سجل المتصفح أو Vercel Runtime Logs. لم تُرسل نماذج ولم تُنشأ بيانات وهمية في الإنتاج. يبقى اختبار قبول بفعالية حقيقية خطوة تشغيلية عند تجهيز الفعالية التالية. |
| Legal launch identity and contact — 2026-08-15 | **In progress** | اعتمدت مكة كمدينة و`bayn.collective@gmail.com` كوسيلة تواصل عامة للخصوصية والعملاء، وأضيفتا إلى صفحات التواصل والخصوصية والشروط. لم تُسجل بيانات الوثيقة الخاصة في Git ولم يُنشر الاسم الشخصي بطلب المالك. يبقى اسم المقر وعنوانه الكامل، والتحقق من ملاءمة النشاط والصفة النظامية وطريقة الإفصاح، والمراجعة القانونية النهائية قبل إزالة تنبيه المسودة. |
...
| Event-centered manual messaging — 2026-08-20 | **Completed in production** | أضيفت migration أمامية `20260820194036_manual_message_workflow.sql` ومساحة «التواصل» داخل كل فعالية لست فئات رسائل مع فتح WhatsApp بخطوة تشغيلية واحدة وتأكيد إرسال منفصل. لا يحفظ السجل بيانات شخصية أو نص الرسالة أو token صريحًا، وتبطل إعادة التجهيز المحاولة غير المرسلة السابقة. نجحت اختبارات التطوير pgTAP (`17/17`) وVitest (`115/115`) والفحوص والبناء. في `2026-08-21` طُبقت migration عبر Supabase وسُجلت إنتاجيًا باسم `20260820210954_manual_message_workflow`. أكد فحص ما بعد التطبيق RLS والصلاحيات والـRPC وثبات فعاليتين و٣ تسجيلات وعدم وجود رسائل وهمية، ثم رُقّي Vercel deployment `dpl_35asjTbvw2uxsULZLBsubuTMScKe` إلى Production. |
| Quick event status control — 2026-08-21 | **Completed in production** | أضيف تحكم «تغيير الحالة» إلى صف الفعالية في `/admin/events` وربط بإجراء الخادم وصلاحياته الحالية. النشر والاستعادة مباشران، والأرشفة والإلغاء خلف تأكيد، والمسودة الناقصة توجّه إلى إكمال البيانات. نجح الاختبار المخصص (`3/3`) ثم `pnpm lint` و`pnpm typecheck` وكامل Vitest (`116/116`) وبناء Next.js `16.2.12`. بعد ترقية `dpl_35asjTbvw2uxsULZLBsubuTMScKe` فُحصت جلسة الإدارة الفعلية، وظهر التحكم وخيارا الأرشفة والإلغاء للفعالية المنشورة دون تنفيذ تعديل اختباري على بيانات الإنتاج، ولم تظهر أخطاء Runtime. |
...
| الاستجابة والوصولية | **In Progress** | أضيفت حالات تركيز ظاهرة، رابط تخطي، أهداف لمس مناسبة، وقائمة جوال سفلية قابلة للتمرير والإغلاق بـEscape. قُسمت نماذج الطلب الطويلة إلى مجموعات وظيفية، وصُغّر الهيدر والعنوان على الجوال، وأصبح الفوتر في أسفل الصفحات القصيرة، وتحول تقويم الإدارة إلى قائمة أيام على الجوال. نجحت مراجعة العرض عند `320×568`, `375×667`, `390×844`, `430×932`, `768×1024`, `1024×768`, و`1440×900` دون overflow أو صور مكسورة، ونجحت حالة رابط الاستعادة غير الصالح عند `375×667`؛ تبقى مراجعة المسارات المتصلة بـSupabase على Preview. |
...
طُبقت migrations التصحيحية واختباراتها أولًا على مشروع التطوير، ثم نُقلت إلى مشروع الإنتاج في نافذة القطع الموثقة أعلاه. ما زالت Preview تستخدم مشروع التطوير، بينما Production يستخدم مشروع الإنتاج.
