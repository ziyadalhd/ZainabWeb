...
- Deferred drafts: the conflicting SQL drafts were excluded from the deployable migration chain. They were removed from the working tree after later forward migrations superseded them; Git history retains the original files. No hosted migration was removed.
...
- Status: implemented on `codex/product-quality-polish`. Standing quality gates passed at the time (`pnpm lint`, `pnpm typecheck`, `pnpm test` — 222 tests across 60 files, `pnpm build`). The original audit remains in Git history; verify current code before relying on its findings.
- Source: the superseded `ADMIN_UX_REDESIGN_PLAN.md` audit is retained in Git history. It diagnosed an unexecuted approved fix to the event-workspace registration table and a navigation choice that split one operator job across two screens. See `ADMIN_OVERHAUL_PLAN.md` §13 for the reconciliation and the approved decisions below.
...
## Admin-only operational alerts via Telegram Bot API: 2026-08-31

- Status: implemented on `codex/product-quality-polish`.
- Scope: this is a narrower, distinct decision from the deferred "Automated WhatsApp" item in `docs/open-questions.md` — that item concerns guest-facing messaging (confirmations, reminders, waitlist invitations) via an official WhatsApp Business API, which remains deferred exactly as documented. This decision covers only a one-way, admin-only operational alert ("a new registration landed") sent to a single administrator's own chat.
- Provider (superseded same day): CallMeBot (an unofficial free WhatsApp relay) was the original provider, approved for this same narrow admin-alert scope. It proved too slow/unreliable in practice and was replaced outright by the official Telegram Bot API (`api.telegram.org`) — no CallMeBot code, env vars, or references remain. `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` are server-only env vars (`.env.example` placeholders added); no token or chat id is exposed to the browser.
- Privacy: the alert text includes the registrant's real name and event title, sent once per event (new registration, guest attendance confirmation, guest cancellation) to the admin's private Telegram chat via the bot. The product owner accepted this tradeoff explicitly for a single trusted admin recipient; no other personal data (phone, email) is included.
- Failure handling: `lib/notifications/telegram.ts` never throws — a missing configuration, non-2xx response, or network failure is logged (status/error code only, no message body) and otherwise silently skipped, so an alert failure can never affect registration, confirmation, or cancellation outcomes.
- Call sites: `app/(public)/events/[id]/actions.ts` (new registration/waitlist), `app/(public)/bookings/[token]/actions.ts` (guest-initiated attendance confirmation and cancellation), both via `lib/notifications/admin-alerts.ts`. Admin-initiated status changes are not alerted, since the admin already caused them.

## Public archive of past events and the admin MFA gate: 2026-09-23

- Status: approved by the product owner and implemented on `codex/product-quality-polish`. The RLS migration `20260922090000_public_past_events.sql` is written but, like `20260921090000_editable_message_templates.sql` and `20260914090000_add_event_audiences.sql`, is applied through the Supabase SQL Editor rather than `supabase db push` (see below).
- Archive rule: an event appears at `/events/past` if and only if it is `published` and `ends_at <= now()`. No new status or flag was added; the existing lifecycle carries the meaning. An administrator hides a trial event that never took place by archiving it, never by deleting it. Deletion was rejected because `registrations` and `event_feedback_links` reference events `on delete restrict`, and archiving is reversible (`archived → draft → published`).
- Trial-data cleanup: registration counts, check-ins, and posters were all checked as signals for which past events took place and none was reliable (the trial event titled `-` holds three check-ins while most real events hold none). The owner classified each past event by hand; two published events that did not take place are to be archived from the dashboard.
- RLS: `events_public_past_select` is an additive permissive policy for `anon, authenticated` exposing only published, ended event rows. No other table's policy changed. `listPastEvents()` applies the same filter in the query itself, because an administrator's session can read every event through `events_authenticated_select`.
- MFA gate: the temporary `ADMIN_MFA_ENABLED` flag was removed and the original gate restored. The flag could not disable MFA: `admin_users` is readable at `aal1`, so the application admitted the session, while `private.is_admin()` requires `aal2`, so the database served it a visitor's view with silently ineffective writes. `lib/auth/require-admin.test.ts` now covers the gate; three of its cases fail against the flagged version.
- Migration history: the Supabase project is shared with an unrelated interactive-wall application whose six migrations exist only remotely, so `supabase db push` refuses to run. Until the applications are separated, new migrations are applied in the SQL Editor and then recorded with `supabase migration repair --status applied <version>`. The wall application's history is left untouched.
- Not verified: `supabase/tests/events_rls.test.sql` was extended with archived and cancelled past fixtures but not executed, because the linked project is production and `pnpm test:db` must not target it.
- Owner-facing operating guide: `docs/operating-model.md` (Arabic).
