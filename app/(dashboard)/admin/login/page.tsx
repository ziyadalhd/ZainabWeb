  ...
  return (
    <main className="grid min-h-screen bg-[var(--brand-cream)] lg:grid-cols-[minmax(0,0.72fr)_minmax(28rem,1fr)]">
      <aside className="hidden bg-[var(--brand-forest)] p-10 text-[var(--color-on-primary)] lg:flex lg:flex-col lg:justify-between">
        <Link href="/" className="w-fit bg-[var(--brand-cream)] p-4" aria-label="العودة إلى موقع نادي بَيْن الثقافي"><ClubLogo className="h-auto w-40" priority /></Link>
        <div className="max-w-sm border-r-4 border-[var(--brand-amber)] pr-5"><p className="text-4xl font-black leading-tight">لوحة إدارة الفعاليات والتسجيلات</p><p className="mt-4 text-sm leading-7 text-white/75">الدخول مخصص للمسؤول المعتمد في نادي بَيْن الثقافي.</p></div>
      </aside>
      <div className="grid place-items-center px-4 py-10 sm:px-8">
        <div className="w-full max-w-md">
          <Link href="/" className="mx-auto flex w-fit items-center lg:hidden" aria-label="العودة إلى موقع نادي بَيْن الثقافي"><ClubLogo className="h-auto w-32" priority /></Link>
