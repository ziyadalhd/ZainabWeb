...
export default async function EditEventPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="px-4 py-8 sm:px-8"><PageHeader eyebrow="إدارة الفعاليات" title="تعديل الفعالية" description={`حالة النشر الحالية: ${event.publicationStatus === "draft" ? "مسودة" : event.publicationStatus === "published" ? "منشورة" : "مؤرشفة"}.`} /><EventForm action={updateEventAction.bind(null, id)} event={event} submitLabel="حفظ التعديلات" /></main>;
}
