...
export default async function EventWorkspacePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ tab?: string; success?: string; error?: string }> }) {
  ...
  const [registrations, feedback] = await Promise.all([
    ...
    tab === "overview" || tab === "feedback" ? feedbackRepository.listSubmittedForEvent(event.id) : Promise.resolve([]),
  ]);
  ...
  const [eventTemplate, globalTemplate, manualMessages] = tab === "communications"
    ? await Promise.all([
  ...
  return (
    <main className="admin-page">
      ...
      <section className="mt-7" aria-labelledby="workspace-content-heading">
        ...
        {tab === "feedback" ? <AdminEventFeedbackTable responses={feedback} /> : null}
