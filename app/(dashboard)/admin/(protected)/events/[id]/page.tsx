...
export default async function EventWorkspacePage({
  ...
  if (!registrationsOutcome.ok) {
    return (
      <main className="admin-page">
        ...
        <PageHeader
          eyebrow="مساحة الفعالية"
          title={event.title}
          description={`${formatArabicEventDate(event.startsAt)} · ${formatArabicEventTimeRange(event.startsAt, event.endsAt)}`}
        />
  ...
  const [eventTemplate, globalTemplate, manualMessagesOutcome] =
    tab === "communications"
      ? await Promise.all([
  ...
  return (
    <main className="admin-page">
      ...
      <div className="workspace-status mt-6">
      ...
      <nav aria-label="أقسام الفعالية" className="workspace-tabs mt-7">
        {tabs.map((item) => (
          <Link
            key={item.id}
            href={`/admin/events/${event.id}?tab=${item.id}`}
      ...
      <section className="mt-7" aria-labelledby="workspace-content-heading">
        <h2 id="workspace-content-heading" className="sr-only">
          {activeTab.label}
        </h2>
        {event.publicationStatus === "cancelled" && tab !== "communications" ? (
          <section className="notice-error mb-6 max-w-3xl p-5" aria-label="إشعار إلغاء الفعالية">
            <h3 className="font-black">الفعالية ملغاة</h3>
            <p className="mt-2">{formatArabicNumber(registered.length)} مسجّلات متأثرات. أرسلي الإشعار الموحد وسجّلي الإرسال من مساحة التواصل.</p>
            <Link href={`/admin/events/${event.id}?tab=communications`} className="button-secondary mt-4">
              فتح إشعارات الإلغاء
            </Link>
          </section>
        ) : null}
        {tab === "overview" ? (
          <div className="grid gap-5 lg:grid-cols-3">
            <article className="card-surface p-5">
              <p className="eyebrow">التسجيلات</p>
              <p className="mt-3 text-3xl font-black">{formatArabicNumber(registered.length)}</p>
              <Link
                href={`/admin/events/${event.id}?tab=registrations`}
                className="mt-3 inline-block text-sm font-bold underline decoration-[var(--brand-amber)] underline-offset-4"
              >
                مراجعة المسجلات
              </Link>
            </article>
            <article className="card-surface p-5">
              ...
              <p className="mt-3 text-3xl font-black">{formatArabicNumber(waitlist.length)}</p>
              <Link
                href={`/admin/events/${event.id}?tab=waitlist`}
                className="mt-3 inline-block text-sm font-bold underline decoration-[var(--brand-amber)] underline-offset-4"
              >
                إدارة الانتظار
              </Link>
            ...
            <article className="card-surface p-5">
              ...
              <p className="mt-3 text-3xl font-black">{feedback ? formatArabicNumber(feedback.length) : "—"}</p>
              <Link
                href={`/admin/events/${event.id}?tab=feedback`}
                className="mt-3 inline-block text-sm font-bold underline decoration-[var(--brand-amber)] underline-offset-4"
              >
                عرض التقييمات
              </Link>
          ...
          manualMessages ? (
            <div className="grid gap-6">
              ...
              <details className="message-history max-w-3xl">
                ...
                <form action={saveEventReminderTemplateAction.bind(null, event.id)} className="border-t border-[var(--color-border)] p-6">
                  ...
                  {success === "template" ? <p className="notice-success mt-4">تم حفظ تخصيص الفعالية.</p> : null}
                  {error === "template" ? (
                    <p role="alert" className="notice-error mt-4">
                      تحققي من النص والمتغيرات المطلوبة.
                    </p>
                  ) : null}
        ...
        {tab === "settings" ? (
          <div className="grid max-w-2xl gap-5">
            <div className="card-surface p-6">
              ...
              <p className="mt-3 muted-copy">عدّلي المحتوى والموعد والسعة وحالة التسجيل من صفحة الإعدادات.</p>
              <Link href={`/admin/events/${event.id}/edit`} className="button-primary mt-5">
                تعديل الإعدادات
              </Link>
            </div>
            <div className="card-surface p-6">
