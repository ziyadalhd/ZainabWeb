...
export default async function WaitlistPage({ searchParams }: { searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  return <main className="admin-page"><div className="flex flex-wrap items-end justify-between gap-5"><PageHeader eyebrow="لوحة الإدارة" title="قائمة الانتظار" description="تظهر الأسماء حسب وقت التسجيل. عند توفر مقعد، اختاري البديلة وأرسلي لها الدعوة؛ تبقى الدعوة متاحة ٦ ساعات." /><Link href="/admin/registrations/export?scope=waitlist" className="button-secondary">تنزيل القائمة</Link></div>{success && successMessages[success] ? <p role="status" className="notice-success mt-6">{successMessages[success]}</p> : null}{error ? <p role="alert" className="notice-error mt-6">تعذر تنفيذ الإجراء. قد تكون المقاعد مكتملة أو تغيّرت حالة التسجيل.</p> : null}<div className="mt-8"><RegistrationTable registrations={registrations} mode="waitlist" /></div></main>;
}
