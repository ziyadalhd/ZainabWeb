...
export default async function WaitlistPage({ searchParams }: { searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  return <main className="px-4 py-8 sm:px-8"><div className="flex flex-wrap items-end justify-between gap-5"><PageHeader eyebrow="لوحة الإدارة" title="قائمة الانتظار" description="الترتيب حسب وقت التسجيل. اختاري البديلة يدويًا بعد توفر مقعد؛ تتحول إلى دعوة لمدة 6 ساعات ولا توجد ترقية تلقائية." /><Link href="/admin/registrations/export?scope=waitlist" className="rounded-2xl border border-[var(--brand-green)] px-5 py-3 font-extrabold text-[var(--brand-green)]">تنزيل CSV</Link></div>{success && successMessages[success] ? <p role="status" className="mt-6 rounded-2xl bg-[var(--color-success-bg)] px-4 py-3 font-bold text-[var(--color-success-text)]">{successMessages[success]}</p> : null}{error ? <p role="alert" className="mt-6 rounded-2xl bg-[var(--color-error-bg)] px-4 py-3 font-bold text-[var(--color-error-text)]">تعذر تنفيذ الإجراء. قد تكون السعة مكتملة أو تغير السجل.</p> : null}<div className="mt-8"><RegistrationTable registrations={registrations} mode="waitlist" /></div></main>;
}
