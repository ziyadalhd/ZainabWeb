...
export default async function RegistrationsPage({ searchParams }: { searchParams: Promise<{ view?: string; id?: string; q?: string; page?: string; success?: string; error?: string }> }) {
  ...
  const outcome = await repository.listPage({ view, query, page: getPage(requestedPage), pageSize, now: new Date().toISOString() });
  ...
  return (
    <main className="admin-page">
      ...
      {outcome.ok ? (
        <>
          <div className="mt-5 flex flex-wrap items-end justify-between gap-4"><form action="/admin/registrations" className="flex min-w-[min(100%,22rem)] flex-1 flex-wrap gap-2"><input type="hidden" name="view" value={view} /><label className="sr-only" htmlFor="registration-search">ابحثي في التسجيلات</label><input id="registration-search" name="q" defaultValue={query} placeholder="الاسم أو الجوال أو الفعالية أو المرجع" className="min-h-11 min-w-0 flex-1 rounded-[var(--radius-control)] border border-[var(--color-border)] bg-[var(--color-surface)] px-3" /><button type="submit" className="button-primary">بحث</button>{query ? <Link href={`/admin/registrations?view=${view}`} className="button-quiet">مسح</Link> : null}</form><p className="data-value text-sm font-bold muted-copy">{formatArabicNumber(outcome.data.total)} نتيجة</p></div>
          <div className="mt-6"><RegistrationTable registrations={outcome.data.items} mode={actionMode} selectedId={id} registrationHref={(registration) => registrationHref(registration, view, query, outcome.data.page)} /></div>
          {outcome.data.total > pageSize ? <nav aria-label="ترقيم صفحات التسجيلات" className="mt-6 flex items-center justify-between gap-3"><p className="text-sm muted-copy">صفحة {formatArabicNumber(outcome.data.page)} من {formatArabicNumber(Math.ceil(outcome.data.total / pageSize))}</p><div className="flex gap-2">{outcome.data.page > 1 ? <Link className="button-secondary" href={pageHref(view, query, outcome.data.page - 1)}>السابقة</Link> : null}{outcome.data.page * pageSize < outcome.data.total ? <Link className="button-secondary" href={pageHref(view, query, outcome.data.page + 1)}>التالية</Link> : null}</div></nav> : null}
        </>
      ) : <LoadErrorNotice />}
    </main>
  );
